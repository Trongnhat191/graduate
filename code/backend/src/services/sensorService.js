    ...
    if (entry < 10) {
        ...
        exec('conda run -n graduate python src/python/detect_plate.py 0 src/photos/entry', async (err, stdout, stderr) => {
            if (err) {
                ...
                try {
                    // check xem biển số đã tồn tại chưa
                    const checkResponse = await axios.get(`http://localhost:6969/api/check-plate/${plate}`);
                    if (checkResponse.data.errCode === 1) {
                        const carId = checkResponse.data.carId;
                        // console.log("carId from existed: ", car_Id);
                        // Tạo Parking Logs
                        await axios.post('http://localhost:6969/api/create-parking-logs', {
                            carId: carId,
                        ...
                        const carId = checkResponse.data.carId;
                        // console.log("carId from not existed: ", car_Id);
