...
export const processSensorData = async ({ entry, exit, slot1, slot2 }) => {
    ...
    if (entry < 10 && !entryHandle) {
        ...
        try {
            ...
            if (stderr) {
                console.log("---------------------");
                console.warn(`[Python Entry] ⚠️ STDERR: ${stderr.trim()}`);
                console.log("---------------------");
            }

            const lines = stdout.trim().split("\n");
            const imageName = lines[0] ? lines[0].trim() : "unknown_entry_image.jpg";
            const plate = lines[lines.length - 1] ? lines[lines.length - 1].trim() : "NOPLATE";
            ...
            try {
                const plateCheckResult = await checkPlateExisted(plate);

                if (plateCheckResult.errCode === 1) { // Biển số đã tồn tại
                    await createParkingLogs(plateCheckResult.carId);
                    console.log(`[Entry Car] ✅ Biển số đã tồn tại: ${plate}`);
                    const ticketExisted = await checkTicketExisted(
                        plateCheckResult.carId
                    );
                    if (ticketExisted.errCode === 1) { // Vé đã tồn tại
                        ...
                        if (currentDate > endDate) {
                            ...
                            await deleteTicketByCarId(plateCheckResult.carId);
                            console.log(
                                `[Create Car] ✅ Đã xóa vé tháng cũ cho biển số ${plate}`
                            );
                            await createNewTicket(plateCheckResult.carId);
                            console.log(
                    ...
                    const carId = createCarResponse.data.carId;
                ...
                console.error(`[Create Car] ❌ Lỗi khi xử lý dữ liệu xe vào: ${apiErr}`);
                // Optionally reset status or handle specific API error states
                currentStatus.ticketTypeIn = "error";
                broadcastStatus(currentStatus);
            }
        } catch (execErr) {
            console.log("---------------------");
            console.error(`[Python Entry] ❌ EXEC ERROR: ${execErr.message}`);
            if (execErr.stderr) {
                console.error(`[Python Entry] ❌ STDERR: ${execErr.stderr.trim()}`);
            }
            console.log("---------------------");
            const stdoutContent = execErr.stdout || "";
            const stdoutLines = stdoutContent.trim().split("\n");
            console.log(stdoutLines);
            currentStatus.currentNumberPlateIn = "error";
            currentStatus.imageIn = stdoutLines[0] ? stdoutLines[0].trim() : "error_entry_image.jpg";
    ...
    if (exit < 10 && !exitHandle) {
        ...
        try {
            ...
            if (stderr) {
                console.log("---------------------");
                console.warn(`[Python Exit] ⚠️ STDERR: ${stderr.trim()}`);
                console.log("---------------------");
            }
            const lines = stdout.trim().split("\n");
            const imageName = lines[0] ? lines[0].trim() : "unknown_exit_image.jpg";
            const plate = lines[lines.length - 1] ? lines[lines.length - 1].trim() : "NOPLATE";

            currentStatus.currentNumberPlateOut = plate;
            currentStatus.imageOut = imageName;
            // Do not broadcast yet, fee and ticketTypeOut depend on subsequent logic
            ...
            try {
                ...
                if (!carId) {
                    ...
                    if (isMonthTicket) {
                        ...
                        const timeDiff = await updateEndDate(carId); // This function calculates days
                        const fee = 10000 * (timeDiff || 1); // Default to 1 day if timeDiff is null/0
                        currentStatus.fee = fee;
                        await updateCheckOutTimeAndFee(carId, fee); // Save checkout time and fee
                        
                        const userId = await userService.findUserIdByNumberPlate(plate);
                        if (userId) {
                            const paymentResponse = await userService.payMoney(userId, fee);
                            console.log(`[Payment Attempt] UserID: ${userId}, Fee: ${fee}, Response: ${paymentResponse.errMessage}`);
                            if (paymentResponse.errCode === 0) {
                                console.log(`[Payment] ✅ ${paymentResponse.errMessage}`);
                            } else {
                                console.log(`[Payment] ⚠️ ${paymentResponse.errMessage}`);
                                // Handle cases like insufficient funds, e.g., log for manual payment
                            }
                        } else {
                            console.log(`[Payment] ⚠️ Không tìm thấy UserID cho biển số ${plate}, không thể trừ tiền tự động.`);
                        }
                ...
                console.error(`[Exit Car] ❌ Lỗi khi xử lý dữ liệu xe ra: ${apiErr}`);
                currentStatus.ticketTypeOut = "error";
                currentStatus.fee = 0;
                broadcastStatus(currentStatus);
            }
        } catch (execErr) {
            console.log("---------------------");
            console.error(`[Python Exit] ❌ EXEC ERROR: ${execErr.message}`);
            if (execErr.stderr) {
                console.error(`[Python Exit] ❌ STDERR: ${execErr.stderr.trim()}`);
            }
            console.log("---------------------");
            const stdoutContent = execErr.stdout || "";
            const stdoutLines = stdoutContent.trim().split("\n");
            ...
            currentStatus.imageOut = stdoutLines[0] ? stdoutLines[0].trim() : "error_exit_image.jpg";
            currentStatus.ticketTypeOut = "";
            currentStatus.fee = 0;
            broadcastStatus(currentStatus);
        }
    ...
    const newSlotStatus = { // Renamed to avoid conflict if currentStatus is used before this
        slot1: slot1 < 50 ? "occupied" : "empty",
        slot2: slot2 < 50 ? "occupied" : "empty",
    };

    const isSlotStatusChanged =
        newSlotStatus.slot1 !== currentStatus.slot1 ||
        newSlotStatus.slot2 !== currentStatus.slot2;

    if (isSlotStatusChanged) {
        currentStatus.slot1 = newSlotStatus.slot1; // Update currentStatus
        currentStatus.slot2 = newSlotStatus.slot2;
        broadcastStatus({ slot1: currentStatus.slot1, slot2: currentStatus.slot2 }); // Broadcast only slot changes or full status
    }
